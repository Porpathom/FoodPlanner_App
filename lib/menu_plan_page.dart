...
Future<void> _loadShowMealPlanDisplay() async {
  ...
  setState(() {
    ...
    // ถ้ามีข้อมูลครบถ้วน ให้ใช้ค่าจาก SharedPreferences
    if (hasData) {
      _showMealPlanDisplay = prefs.getBool('showMealPlanDisplay') ?? false;
    } else {
...
Future<void> _loadUserData() async {
  ...
  try {
    ...
    if (user != null) {
      ...
      if (doc.exists) {
        ...
        // หลังจากโหลดข้อมูลเสร็จ ให้โหลดสถานะการแสดงผล
        await _loadShowMealPlanDisplay();

        print("✅ User data loaded successfully: $userData, hasData: $hasData");
      } else {
        setState(() {
          userData = null;
          hasData = false;
          isLoading = false;
          _showMealPlanDisplay = false; // ไม่มีข้อมูล ต้องรีเซ็ตเป็น false
        });
    ...
    setState(() {
      hasData = false;
      isLoading = false;
      _showMealPlanDisplay = false; // เกิดข้อผิดพลาด ต้องรีเซ็ตเป็น false
    });
  }
}
 ...
 Future<void> _navigateToEditPage() async {
  ...
  String breakfastTime = userData?['breakfastTime'] ?? '';
  String lunchTime = userData?['lunchTime'] ?? '';
  String dinnerTime = userData?['dinnerTime'] ?? '';
  String medicalCondition = userData?['medicalCondition'] ?? 'ไม่มีโรคประจำตัว';
  ...
  try {
    ...
    await Future.microtask(() async {
      ...
      // เรียก Navigator.push และรอผลลัพธ์
      ...
      // ตรวจสอบว่า Widget ยังคง mounted อยู่
      if (!mounted) return;

      print("🔄 Returning from EditUserDataPage...");
      await _loadUserData();
...
Widget build(BuildContext context) {
  ...
  Widget _showContent() {
    ...
    if (_showMealPlanDisplay) {
      return MealPlanDisplayPage(
        ...
        healthCondition: userData?['medicalCondition'] ?? 'ไม่มีโรคประจำตัว', // เพิ่มบรรทัดนี้
      );
    }
  ...
  onPressed: _navigateToEditPage,  // เปลี่ยนเป็นใช้ _navigateToEditPage
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: Row(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          const Icon(Icons.health_and_safety, color: Colors.red, size: 22),
            ...
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: ElevatedButton(
                      onPressed: _showMealPlanner, // แทนที่ด้วยการเปลี่ยนสถานะใน State
                      style: ElevatedButton.styleFrom(
                        padding: const EdgeInsets.symmetric(vertical: 16),
