...
void initState() {
  ...
  print("Converted to database key: ${_getConditionKey(widget.healthCondition)}");
...
Future<void> _checkMenuItems() async {
  try {
    QuerySnapshot menuCount = await _firestore
...
Future<void> _updateMealMenu(int dayIndex, String mealType, Map<String, dynamic> newMenu) async {
  try {
    if (currentMealPlanId == null) {
      throw Exception('ไม่พบ ID ของแผนอาหาร');
    }

    // สร้างข้อมูลเมนูสำหรับการอัปเดต
  ...
  try {
    ...
    if (user != null) {
      ...
      if (activePlanSnapshot.docs.isNotEmpty) {
        ...
        print("No active meal plan found, checking if we can create a new plan");
        ...
        if (canCreatePlan) {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text('สร้างแผนอาหารใหม่สำหรับผู้${currentHealthCondition == 'healthy' ? 'ไม่มีโรคประจำตัว' : 'มีสภาวะ $currentHealthCondition'}'),
          ...
          if (mounted) {
            await showDialog(
              ...
              builder: (context) => AlertDialog(
                ...
                content: Text('ไม่มีเมนูอาหารที่เหมาะสมสำหรับผู้มีสภาวะ $currentHealthCondition เพียงพอสำหรับสร้างแผนอาหาร 7 วัน\n\nต้องมีเมนูอย่างน้อย 7 เมนูสำหรับแต่ละมื้อ'),
                actions: [
                  TextButton(
                    onPressed: () {
...
String _getConditionKey(String condition) {
  ...
  switch (condition.toLowerCase()) {
    ...
    case 'heartdisease':
      return 'heartDisease';
    default:
      // ถ้าไม่ตรงกับคีย์ใดๆ ให้ใช้ healthy เป็นค่าเริ่มต้น
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        showDialog(
          ...
          builder: (context) => AlertDialog(
            ...
            content: const Text('ไม่พบแผนอาหารที่ใช้งานอยู่ ต้องการสร้างแผนอาหารใหม่หรือไม่?'),
            actions: [
              TextButton(
  ...
  try {
    ...
    if (user != null) {
      ...
      if (breakfastMenus.length < 7 || lunchMenus.length < 7 || dinnerMenus.length < 7) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('ไม่มีเมนูเพียงพอสำหรับผู้มีสภาวะ $currentHealthCondition กรุณาเพิ่มเมนูอาหารก่อน'),
            duration: const Duration(seconds: 3),
          ),
        );
        return;
...
Future<void> _createMealPlanInFirestore(String userId, String healthCondition) async {
  try {
    // แปลงค่า healthCondition เป็นคีย์ที่ใช้จริงในฐานข้อมูล
    String conditionKey = _getConditionKey(healthCondition);
    print("Creating meal plan with condition: $healthCondition (key: $conditionKey)");
    
    // ตรวจสอบอีกครั้งว่ามีเมนูเพียงพอหรือไม่
    bool hasEnoughMenus = await _checkIfEnoughMenuItems();
    if (!hasEnoughMenus) {
      throw Exception('ไม่มีเมนูเพียงพอสำหรับสร้างแผนอาหาร 7 วัน กรุณาเพิ่มเมนูอาหาร');
    }
    
    // ปรับแผนอาหารเก่าให้ไม่แอคทีฟ
    // (โค้ดส่วนนี้ไม่ต้องเปลี่ยน)
    ...
    if (breakfastMenusQuery.docs.length < 7 || lunchMenusQuery.docs.length < 7 || dinnerMenusQuery.docs.length < 7) {
    ...
    Map<String, dynamic> newMealPlan = {
      ...
      'description': 'แผนอาหารสำหรับผู้ที่${healthCondition == 'healthy' ? 'ไม่มีโรคประจำตัว' : 'มีสภาวะ $healthCondition'}',
      'dailyPlans': newDailyPlans,
      'isActive': true
    };
    
    // บันทึกแผนอาหารใน Firestore
    DocumentReference newPlanRef = await _firestore.collection('mealPlans').add(newMealPlan);
  ...
  void _updateLocalMealCompletion(int dayIndex, String mealType, bool isCompleted) {
  if (mounted) {
    setState(() {
      ...
      child: Row(
        children: [
          ...
          IconButton(
            ...
            onPressed: () {
  _updateLocalMealCompletion(dayIndex, mealType, !isCompleted);
},
          ),
  ...
  final selectedMenu = await Navigator.push(
    ...
    MaterialPageRoute(
      builder: (context) => MealSelectionPage(
        healthCondition: currentHealthCondition, // เปลี่ยนจาก healthCondition เป็น currentHealthCondition
        mealType: mealType,
        currentMealId: currentMealId ?? '',
        mealPlanId: currentMealPlanId ?? '', // เปลี่ยนจาก mealPlanId เป็น currentMealPlanId
        dayIndex: dayIndex,
      ),
    ),
  );
...
const SizedBox(height: 12),
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  onPressed: () {
  bool currentStatus = dailyPlans[dayIndex]['completed'][mealType] ?? false;
  _updateLocalMealCompletion(dayIndex, mealType, !currentStatus);
  Navigator.pop(context);
},
  ...
  style: const TextStyle(color: Colors.white),
                  ...
                  style: ElevatedButton.styleFrom(
                    padding: const EdgeInsets.symmetric(vertical: 12),
                    backgroundColor: dailyPlans[dayIndex]['completed'][mealType] ?? false
                        ? Colors.red.shade400
                        : Colors.green,
                  ),
                ),
            ...
            child: Row(
              children: [
                ...
                if (i + 1 < entries.length)
                  Expanded(
                    child: _buildNutrientItem(entries[i + 1].key, entries[i + 1].value),
                  ),
              ],
            ),
          ),
